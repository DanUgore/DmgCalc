Calc = {
	...
	calcDamageNumbers: function (attacker, defender, move, field, isCrit) {
	...
	toModifier: function (value, numerator, denominator) {
	...
	modify: function (value, mod) {
		mod = mod || 0x1000;
		return Math.floor((value * mod + 0x800 - 1) / 0x1000);
	},

	chainModifiers: function (mod1, mod2) {
		mod1 = mod1 || 0x1000;
		mod2 = mod2 || 0x1000;
		return ((mod1 * mod2 + 0x800) >>> 12); // M'' = ((M * M') + 0x800) >> 12
	},

	compareTypes: function (oType, dType) {
		var eff = 1;
	...
	*/
	...
	get: function (handle, returnArray) {
	...
	getFrom: function (handle, fromObj, returnObj) {
		if (!this.calcing) return null;
		handle = handle || '';
	...
	getTypeEff: function (oType, dTypes) {
	...
	getMod: function (modName) { // Responsible for chaining modifiers as well
		modName = modName || '';
	...
	getStat: function (statName, stat, boost, modify) {
		if (boost) {
...
/*
get: function (handle, returnArray) { // bool returnArray - if true, return array of values
	if (!this.calcing) return null;
	handle = handle || '';
